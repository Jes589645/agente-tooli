...
function getEnv(name: string): string {
  return cleanEnv(process.env[name] || loadFallbackEnv()[name] || '')
}
...
export async function GET(_request: NextRequest, context: { params: { path?: string[] } }) {
  try {
    ...
    if (path[0] === 'healthz') {
      return NextResponse.json({ ok: true, model: getEnv('LLM_MODEL') || 'knowledge-base' })
    }
    ...
    return NextResponse.json({
      ok: false,
      error: `Error interno del gateway TOOLI: ${String(error)}`,
    }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest, context: { params: { path?: string[] } }) {
  try {
    ...
    return NextResponse.json({
      ok: false,
      mode: 'error',
      error: `Error interno del gateway TOOLI. Detalle: ${String(error)}`,
    }, { status: 500 })
  }
}
...
async function chatCompletion(userText: string, payload: ChatPayload): Promise<string> {
  ...
  const baseUrl = getEnv('LLM_BASE_URL') || 'https://api.groq.com/openai/v1'
  const model = getEnv('LLM_MODEL') || 'llama-3.1-8b-instant'
  ...
  const response = await fetch(`${baseUrl.replace(/\/$/, '')}/chat/completions`, {
    ...
    body: JSON.stringify({
      model,
      messages,
      temperature: Number(getEnv('TEMPERATURE') || '0.2'),
      top_p: Number(getEnv('TOP_P') || '0.9'),
      max_tokens: Number(getEnv('MAX_TOKENS') || '512'),
    }),
  })
...
async function createTicket(args: TicketArgs) {
  const apiUrl = normalizeGlpiApiUrl(getEnv('GLPI_API_URL'))
